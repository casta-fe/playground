...
import { useModalContext } from "@casta-fe-playground/components/Modal";
import { onMountedOrActivated } from "@casta-fe-playground/hooks";
import { useWindowSizeFn } from "@casta-fe-playground/hooks";
import { getViewportOffset, isBoolean } from "@casta-fe-playground/utils";
  ...
  async function calcTableHeight() {
    ...
    if (!slots.footer) {
      if (tableData.length === 0) {
        //处理空数据时滚动条消失问题
        const TbodyEl = bodyEl.querySelector(".ant-table-tbody");
