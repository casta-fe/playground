...
<script lang="ts">
  ...
  import { buildUUID, isFunction, warn } from "@casta-fe-playground/utils";
    ...
    setup(props, { emit }) {
      ...
      function beforeUpload(file: File) {
        ...
        if (maxSize && file.size / 1024 / 1024 >= maxSize) {
          createMessage.error(
            "只能上传不超过{0}MB的文件!".replace("{0}", `${maxSize}`)
            );
          return false;
        }
      ...
      async function handleStartUpload() {
        ...
        if ((fileListRef.value.length + props.previewFileList?.length ?? 0) > maxNumber) {
          return createMessage.warning(
            "最多只能上传{0}个文件".replace("{0}", `${maxNumber}`)
            );
        }
        try {
      ...
      function handleOk() {
        ...
        if (fileListRef.value.length > maxNumber) {
          return createMessage.warning("最多只能上传{0}个文件".replace("{0}", `${maxNumber}`));
        }
