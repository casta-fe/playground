<script lang="tsx">
  ...
  import { BasicHelp } from "@casta-fe-playground/components/Basic";
  ...
  import {
    ...
    error,
    getSlot,
    formatToDate, getMomentFormatString
  } from "@casta-fe-playground/utils";
    ...
    setup(props, { slots }) {
      ...
      function setEditableFormItemValue(schema, _value) {
        ...
        if (isString(value)) {
          ...
          if (editableComponentSelectTypeMap.has(schema.component)) {
            ...
            // 修复日期不能正常格式化的问题
            let valueFormat = schema.componentProps.valueFormat;
            if (value && value != "-") {
      ...
      function showEditableDom(componentMap, schema) {
        ...
        if (schema.required) {
          ...
          //   debugger;
          // }
          let schemaValue = props.formModel[schema.field];
          //  添加针对0的兼容 by hyb
      ...
      function renderComponent() {
        ...
        // ::==================== i7eo：更新 ///// start  ///// ====================:: //
        const setMaxLengthComponentNames = [
          "Input",
          "InputPassword",
          "InputSearch",
          "InputTextArea"
        ]; // 可以设置默认字符数的组件
        if (setMaxLengthComponentNames.includes(component)) {
          if (component === "InputTextArea") {
            compAttr["maxlength"] = componentProps?.maxLength ?? 256;
            compAttr["showCount"] = componentProps?.showCount ?? true;
            compAttr["autoSize"] = componentProps?.autoSize ?? { minRows: 4, maxRows: 4 };
          } else {
            compAttr["maxlength"] = 32;
          }
        }
        if (component === "InputNumber") {
          compAttr["max"] = componentProps?.max ?? 9999999999;
          compAttr["min"] = componentProps?.min ?? 0;
          compAttr["precision"] = componentProps?.precision ?? 2;
        }

        if (unref(hasEditable)) {
          if (editableComponentSelectTypeMap.has(component)) {
            compAttr["autofocus"] = true;
            compAttr["defaultOpen"] = true;
            compAttr["getPopupContainer"] = () => unref(itemRef);
          } else if (editableComponentTimeTypeMap.has(component)) {
            compAttr["open"] = true;
