  ...
  export default defineComponent({
    ...
    setup(props, { slots }) {
      ...
      function setEditableFormItemValue(schema, _value) {
        ...
        if (isString(value)) {
          ...
          if (
            ...
            const target = schema.componentProps?.options?.reduce((result, option) => {
          ...
          editableItemValue.value = itemValue.value || "-";
        }
      }
      ...
      function showEditableDom(componentMap, schema) {
        ...
        if (schema.required) {
          if (props.formModel[schema.field]) {
            hide();
          } else {
            // 必填项此时无值不能隐藏表单项
      ...
      function renderComponent() {
        ...
        // if(component.includes("Select") && props.schema.componentProps && props.schema.componentProps["mode"] === "multiple") {
        //   compAttr["open"] = true;
        // }
