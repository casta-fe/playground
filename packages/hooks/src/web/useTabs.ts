...
// import emitter from "/@/hooks/web/useEmiiter";
...
//     emitter.emit("tab:set-title");
//     // // fix `ant-tabs-ink-bar` width
//     // await nextTick();
//     // const el: HTMLElement | null = document.querySelector(".ta-multiple-tabs .ant-tabs-ink-bar");
//     // const styleSplitRes = (el?.style?.transform?.match(/\((.*)\)/) ?? [])[1].split(",");
//     // // padding 12px
//     // if (styleSplitRes.length && el) el.style.width = `calc(100% - 12px - ${styleSplitRes[0]})`;
//   }
...
// export function handleDetailTabContent(route, title, setTitle) {
